...
####################### Customize ############################
# For certain country, its transferred certification may not verify for band4
...
ifeq ($(CONFIG_PLATFORM_IMX8), y)
EXTRA_CFLAGS += -DCONFIG_LITTLE_ENDIAN
EXTRA_CFLAGS += -DCONFIG_IOCTL_CFG80211 -DRTW_USE_CFG80211_STA_EVENT

#SUBARCH := $(shell uname -m | sed -e s/i.86/i386/)
ARCH ?= arm64
CROSS_COMPILE ?= /driver/arm/gcc-arm-8.3-2019.03-x86_64-aarch64-linux-gnu/bin/aarch64-linux-gnu-
#KVER  := $(shell uname -r)
#KSRC := /lib/modules/$(KVER)/build
KSRC ?= /workspace/linux-imx8
#MODDESTDIR := /lib/modules/$(KVER)/kernel/drivers/net/wireless/
INSTALL_PREFIX :=
#STAGINGMODDIR := /lib/modules/$(KVER)/kernel/drivers/staging
endif

ifeq ($(CONFIG_PLATFORM_IMX8_ANDROID_11), y)
EXTRA_CFLAGS += -DCONFIG_LITTLE_ENDIAN
EXTRA_CFLAGS += -DCONFIG_IOCTL_CFG80211 -DRTW_USE_CFG80211_STA_EVENT
EXTRA_CFLAGS += -DCONFIG_CONCURRENT_MODE

ARCH ?= arm64
CROSS_COMPILE ?= /w2/boundary_android_11/prebuilts/toolchains/gcc-arm-10.3-2021.07-x86_64-aarch64-none-linux-gnu/bin/aarch64-none-linux-gnu-
KSRC ?= /w2/boundary_android_11/out/target/product/nitrogen8m/obj/KERNEL_OBJ
#MODDESTDIR := /lib/modules/$(KVER)/kernel/drivers/net/wireless/
INSTALL_PREFIX :=
#STAGINGMODDIR := /lib/modules/$(KVER)/kernel/drivers/staging

PATH := /w2/boundary_android_11/prebuilts/toolchains/clang-r428724/bin:/w2/boundary_android_11/prebuilts/misc/linux-x86/lz4:${PATH}
...
ifeq ($(CONFIG_PLATFORM_IMX8_ANDROID_12), y)
EXTRA_CFLAGS += -DCONFIG_LITTLE_ENDIAN
EXTRA_CFLAGS += -DCONFIG_IOCTL_CFG80211 -DRTW_USE_CFG80211_STA_EVENT
EXTRA_CFLAGS += -DCONFIG_CONCURRENT_MODE

ARCH ?= arm64
CROSS_COMPILE ?= /w2/boundary_android_12/prebuilts/toolchains/gcc-arm-10.3-2021.07-x86_64-aarch64-none-linux-gnu/bin/aarch64-none-linux-gnu-
KSRC ?= /w2/boundary_android_12/out/target/product/nitrogen8m/obj/KERNEL_OBJ
#MODDESTDIR := /lib/modules/$(KVER)/kernel/drivers/net/wireless/
INSTALL_PREFIX :=
#STAGINGMODDIR := /lib/modules/$(KVER)/kernel/drivers/staging

PATH := /w2/boundary_android_12/prebuilts/clang/host/linux-x86/clang-r416183b/bin:/w2/boundary_android_12/prebuilts/misc/linux-x86/lz4:${PATH}
CLANG_TRIPLE := $(CROSS_COMPILE)
CCACHE_NODIRECT ?= "true"
LLVM ?= 1
LLVM_IAS ?= 1
...
modules:
ifeq ($(shell test $(CONFIG_RTW_ANDROID) -gt 0; echo $$?), 0)
#$(MAKE) ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE) LLVM=$(LLVM) LLVM_IAS=$(LLVM_IAS) -C $(KSRC) M=$(shell pwd) KCFLAGS="-Wno-incompatible-pointer-types" KAFLAGS="" modules
	$(MAKE) ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE) LLVM=$(LLVM) LLVM_IAS=$(LLVM_IAS) -C $(KSRC) M=$(shell pwd) modules
else
